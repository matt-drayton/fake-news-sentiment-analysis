...
def load_results(model):
    ...
    return pd.DataFrame.from_dict(classifications_fake), pd.DataFrame.from_dict(
        classifications_real
    )
...
@app.callback(Output("graph", "figure"), [Input("model-select", "value")])
